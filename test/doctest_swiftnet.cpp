...
void test_grads(sycl::queue &q, const int input_width, const int output_width, const int n_hidden_layers,
    ...
    sycl::half input_val_half = 0.01;
    sycl::half param_half = 0.01;
    std::cout << "Val: " << input_val_half * param_half * 16 * param_half * 16 * param_half * 16 << ", "
              << input_val_half * param_half * 16 * param_half * 16 << ", " << input_val_half * param_half * 16
              << std::endl;
    ...
    // q.wait();
    // printVector("interm_backw_vec: ", interm_backw_vec, 128, -1);
...
TEST_CASE("Swiftnet - test grad unpadded") {
    ...
    auto test_function = [=](sycl::queue &q, const int width, const int batch_size, std::string activation,
        ...
        typedef sycl::half T;
        T input_val = 4.096e-5;
        std::cout << "Input val: " << input_val << std::endl;
        if (width == 16)
    ...
    // std::array<decltype(bf16_type), 2> types = {bf16_type, half_type};
    // std::array<decltype(half_type), 1> types = {half_type};
    // for (auto type : types) {
    // auto type = half_type;
    // auto type = bf16_type;
    // std::cout << (bf16_type == half_type) << std::endl;
    // std::string type_name = (type == bf16_type) ? "bfloat16" : "half";
    for (int batch_size : batch_sizes) {
        for (int width : widths) {
            for (std::string activation : activations) {
